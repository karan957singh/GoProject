...
func getAirportHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := db.Query("select * from movies")
			...
			// moviesCol := []models.Movies{}
			for rows.Next() {
				rows.Scan(&airports.Id, &airports.Name, &airports.City, &airports.Country, &airports.Iata_code, &airports.Status, &airports.Created_at, &airports.Updated_at, &airports.Deleted_at)

				// moviesCol = append(moviesCol, movies)
			}
			if airports.Id == 0 {
				http.Error(w, "airport not found", 404)
				return
			}
	...
	// 		// moviesCol := []models.Movies{}
	// 		for rows.Next() {
	// 			rows.Scan(&airports.Id, &airports.Name, &airports.City, &airports.Contry, &airports.Iata_code, &airports.Status, &airports.Created_at, &airports.Updated_at, &airports.Deleted_at)
...
func saveMovieHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(params["id"]) > 0 {
		...
		if movies.Genre_id != 0 && movies.Title != "" {
			rowsAffected, _ := db.Exec("insert into movies(genre_id, title, numberInStock, dailyRentalRate, liked) values(?, ?, ?, ?, ?)", movies.Genre_id, movies.Title, movies.NumberInStock, movies.DailyRentalRate, movies.Liked)
			fmt.Println("rows affected: ", rowsAffected)
		}

		// affCount, err := rowsAffected.RowsAffected()
		// if err == nil {
		...
		// if affCount == 0 {
		// 	http.Error(w, "Unable to create movie", 404)
		// 	return
		// }
	}
}
...
func registerUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("register user handler called!")
	params := r.URL.Query()
	paramsBody := r.Body
	json.NewDecoder(paramsBody).Decode(&user)
	fmt.Println("URL params: ", params, " Body: ", user)
}
...
func main() {
	...
	http.HandleFunc("/save-user", registerUserHandler)
	// http.Handle("/", http.FileServer(http.Dir("./build/")))
	http.Handle("/", http.FileServer(http.Dir("build/")))

	// http.Handle("/", http.FileServer(http.Dir("../../react_project/react-app/build/")))
	// http.Handle("/", http.FileServer(http.Dir("../../react_project/react-app/public/")))
	http.ListenAndServe(":3000", nil)
	fmt.Println("Connection successful")
}

// http.FileServer(http.Dir("../../react_project/react-app/build"))
